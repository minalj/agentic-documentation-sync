...
This boundary verifies that repositories are explicitly authorized and that the request contains a valid repository reference and branch or commit where provided. It also treats repository content as untrusted input and applies validation before content is passed to agents or external operations.
...
Agents communicate through structured, execution-scoped messages. Each message carries the execution ID, repository identifier, branch or commit, stage, timestamp, and redacted diagnostics where applicable.
...
The orchestrator passes outputs between agents and does not allow an agent to bypass the approval and validation gate. Sanitization and redaction occur before model or external-tool input boundaries.
...
Repository size, file-count, file-size, API-call, CPU, memory, storage, and processing-time limits are configurable. A repository with no usable content is reported as `Unsupported`; access and processing failures receive the required repository-level status.
...
4. Locate the matching documentation page.
5. Create the page when no matching page exists; otherwise update the existing page.
6. Preserve the authorized Confluence access restrictions.
7. Record page ID, create/update operation, publishing result, HTTP status, errors, retries, and completion time.

Publishing is triggered automatically only after explicit approval and successful validation. No publishing occurs before approval. Transient failures use limited retries with exponential backoff; authentication and permission failures are not repeatedly retried.
